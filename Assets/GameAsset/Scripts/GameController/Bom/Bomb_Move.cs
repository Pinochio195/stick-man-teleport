...
public class Bomb_Move : MonoBehaviour
{
    ...
    public MeshRenderer BomMeshRenderer;
    public float speed = 5f; // tốc độ di chuyển
    private Rigidbody rb;
    private Vector3 direction;
    public bool BOOM_BOM;
    private void Start()
    {
    ...
    {
        rb.velocity = direction * 250 * Time.deltaTime;
    }
    ...
    public void NoLan()
    {
        gameObject.GetComponent<BoxCollider>().enabled = false;
            ...
            HashSet<GameObject> enemiesFound = new HashSet<GameObject>();
            foreach (Collider hit in colliders)
            {
                ...
                if (hit.gameObject.CompareTag("Die_Other"))
                {
                    ...
                    // Nếu enemy chưa được tìm thấy trước đó
                    if (!enemiesFound.Contains(enemy.gameObject))
                    {
                        Debug.Log(enemy.name);
                        enemy.GetComponentsInChildren<Transform>().ToList().ForEach(item=>item.tag="Untagged");
                        StartCoroutine(Blink(enemy.transform.GetChild(0).GetComponent<SkinnedMeshRenderer>(),
                        enemy));
                        enemiesFound.Add(enemy.gameObject);
                        ...
                        if (enemy.GetComponent<ZombieMove>() != null)
                        {
                            ...
                            enemy.GetComponent<ZombieMove>().enabled = false;
                        }
                        ...
                        {
                            if (enemy.gameObject.GetInstanceID() ==
                        ...
                        for (int i = 0; i < count; i++)
                        {
                            ...
                            {
                                GameController.Instance.list_GO_imageEnemeDied[i].GetComponent<Image>().color =
                                    Color.gray;
    ...
    void OnTriggerEnter(Collider other)
    {
        ...
        if (other.gameObject.CompareTag("Weapon"))
        {
            ...
            {
                //lấy ra enemy nằm trong vùng và cho nó nổ
                if (hit.gameObject.CompareTag("Boom"))
                {
                    hit.gameObject.GetComponent<Bomb_Move>().BOOM_BOM = true;
                }
                if (hit.gameObject.CompareTag("Die_Other"))
                {
                    Transform enemy = PlayerController.GetTopLevelParent(hit.transform);
                    ...
                    if (!enemiesFound.Contains(enemy.gameObject))
                    {
                        ...
                        {
                            enemy.GetComponent<ZombieMove>().enabled = false;
                            Destroy(enemy.GetComponent<ZombieMove>());//xóa hẳn để không vào trong script nữa
                        }
